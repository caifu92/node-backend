...
const {
  GL_REPORT_MODIFIED_JOB_KEY: JOB_KEY,
  INITIAL_GL_REPORT_MODIFIED_JOB: INITIAL_JOB,
...
const job = async (realmID) => {
  try {
    ...
    const startModdate = moment().format('YYYY-MM-DD');
    const endModdate = moment().format('YYYY-MM-DD');

    const options = quickBook => ({
      headers: {
      ...
      json: true,
      url: `${config.qboapi_uri}${quickBook.realmID}/reports/GeneralLedger?start_date=${moment().add(-3, 'year').add(1, 'day').format('YYYY-MM-DD')}&end_date=${moment().add(3, 'year').format('YYYY-MM-DD')}&start_moddate=${startModdate}&end_moddate=${endModdate}&columns=tx_date,account_type,txn_type,doc_num,cust_name,name,emp_name,vend_name,item_name,memo,account_name,klass_name,net_amount,subt_nat_home_amount,debt_home_amt,credit_home_amt,credit_amt,debt_amt,currency,is_adj,tax_code&account_type=Income,CostOfGoodsSold,Expense,OtherIncome&minorVersion=38`,
    });
    ...
    await Promise.all([
      ...
      .then(async (data) => {
        try {
          ...
          const tranIDs = allReports.map(item => item.tranID);
          // TODO: Create if not found and Update if found

          await Promise.all([
            ...tranIDs.filter(tranID => !!tranID).map(tranID => QuickbookGLReportRow.destroy({
              where: { tranID },
            })),
          ]);
